...
d3.linegraph = function(noTicks, noDots, parties, partyColors, partyNames, dataMax, dataMin, additionalMonths) {
    /* params */
    if (!parties) {
        parties = ['spd', 'sapd', 'uspd', 'ddp', 'z', 'dvp', 'dnvp', 'nsdap', 'other'];
    }
    if (!partyColors) {
    ...
    function linegraph(dataset) {
     dataset.each(function (data) {
      ...
      const xScale = d3.scaleUtc([new Date(1925, 0), addMonths(maxDate, additionalMonths)], [marginLeft, width - marginRight]);
